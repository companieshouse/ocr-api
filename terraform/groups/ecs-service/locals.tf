...
locals {
  stack_name                 = "ocr" # this must match the stack name the service deploys into
  ...
  healthcheck_path           = "/ocr-api/healthcheck" # healthcheck path for sanctions search service api
  healthcheck_matcher        = "200"
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "ocr-api.env"
